...
try:
    ...
    print("❌ Missing tenacity library. Please run: pip install tenacity")
    exit(1)
...
def robust_llm_invoke(llm, prompt_payload):
    """Wraps LLM calls to handle Azure 429 Rate Limits with exponential backoff."""
    return llm.invoke(prompt_payload)
...
def segment_steps(steps: List[dict]) -> List[List[dict]]:
    ...
    try:
        response = robust_llm_invoke(llm, [system_prompt, HumanMessage(content=steps_text)])
...
def domain_analyzer_node(state: AgentState):
    ...
    try:
        response = robust_llm_invoke(llm, [system_prompt, user_prompt])
...
def predictor_node(state: AgentState):
    ...
    system_prompt = SystemMessage(content=(
        ...
        '    {"step_id_checkpoint": "END", "what_to_verify": "Verify the Alerts Summary grid has successfully re-rendered to display only inventory data relevant to the selected Team, and check if the Monthly Trend chart updated its forecasting lines."}\n'
        '  ]\n'
        "}"
    ))
    
    try:
        response = robust_llm_invoke(llm, [system_prompt, HumanMessage(content=content_payload)])
        raw_json = response.content.strip().replace("```json", "").replace("```", "").strip()
        ...
        print(f"    └── Intent: {result.get('chunk_business_intent', 'Unknown')}")
...
def verifier_node(state: AgentState):
    ...
    for task in state['verification_plan']:
        ...
        system_prompt = SystemMessage(content=(
            ...
            "Output JSON: { \"status\": \"PASS\" or \"FAIL\", \"description\": \"Detailed business-aware explanation\" }"
        ))
        user_prompt = HumanMessage(content=[
...
# --- NEW: Checkpointing Helper ---
def export_excel_checkpoint(excel_rows, excel_report_path, total_cost):
    """Safely dumps the ongoing test report to disk after every chunk."""
    if not excel_rows:
        return
    ...
    with pd.ExcelWriter(excel_report_path, engine='openpyxl') as writer:
        ...
        col_widths = {'A': 25, 'B': 15, 'C': 45, 'D': 45, 'E': 45, 'F': 55, 'G': 15, 'H': 15}
...
def main():
    ...
    for section in kb.get("sections", []):
        ...
        for chunk_idx, chunk_steps in enumerate(chunks):
            ...
            # --- NEW: Sliding V-DOM Context Strategy ---
            # Prevent the UI graph from growing infinitely and blowing up the token context window
            MAX_VDOM_KEYS = 15
            if isinstance(ongoing_ui_graph, dict) and len(ongoing_ui_graph) > MAX_VDOM_KEYS:
                # Keep only the last 15 elements 
                ongoing_ui_graph = {k: ongoing_ui_graph[k] for k in list(ongoing_ui_graph.keys())[-MAX_VDOM_KEYS:]}
            ...
            with get_openai_callback() as cb:
                ...
                # Accumulate the costs for this chunk
                chunk_cost = cb.total_cost
                total_cost += chunk_cost
                total_tokens += cb.total_tokens
                
                if VERBOSE_DEBUG_MODE:
                    print(f"  💰 [Cost Tracker] Chunk Cost: ${chunk_cost:.4f} ({cb.total_tokens} tokens)")
            ...
            # --- NEW: Checkpoint Excel Save ---
            export_excel_checkpoint(excel_rows, excel_report_path, total_cost)
