...
class AgentState(TypedDict):
    ...
    domain_context: str  # <--- NEW: Holds business context
...
# --- NEW NODE: Domain Analyzer (The "Gestalt" Pass) ---
def domain_analyzer_node(state: AgentState):
    ...
    try:
        ...
        # Save to global memory so we don't re-run this for every chunk
...
def predictor_node(state: AgentState):
    ...
    # 🔴 THE FIX: Radically altered prompt to force Business-Level QA reasoning
    system_prompt = SystemMessage(content=(
...
def main():
    ...
    if excel_rows:
        ...
        with pd.ExcelWriter(excel_report_path, engine='openpyxl') as writer:
            ...
            # --- Apply Data Cell Styles (Wrapping & Borders) ---
            ...
            for cell in worksheet[worksheet.max_row]:
                ...
                cell.fill = PatternFill("solid", fgColor="D9D9D9") # Light Gray
